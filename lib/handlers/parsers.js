...
export async function handleDebtParse(ctx) {
  ...
  try {
    ...
    logger.error(err, 'Debt parsing error');
...
export async function handleTransactionParse(ctx) {
  ...
  try {
    ...
    // A bare amount with a valid verb (e.g. "vendi 1000", "gastei 500") is a
    // legitimate entry — default the description instead of rejecting. The regex
    // parser returns a non-ambiguous result for these, so without this default
    // they'd never reach the OpenAI fallback and would be rejected with a
    // confusing "didn't understand" despite being complete, valid input.
    const rawDesc = typeof parsed.description === 'string' ? parsed.description.trim() : '';
    parsed.description = rawDesc || (parsed.type === 'income' ? 'venda' : 'gasto');
    ...
    logger.error(err, 'Transaction parsing error');
