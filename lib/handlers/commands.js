import { SessionState, isValidDebtName, formatKz, isAffirmative, isNegative, hashPhone, getAngolaMidnightUTC, ANGOLA_OFFSET_MS, MAX_AMOUNT } from '../security.js';
...
export async function handleStats(ctx) {
  ...
  try {
    ...
    console.error('[Stats] computeDailyMetrics error:', err.message, err.stack);
  ...
  try {
    ...
    console.error('[Stats] Failed to load yesterday snapshot:', err.message, err.stack);
  }
...
Sistema: ${uptimeDays}d ${uptimeHours}h ${uptimeMins}m | MongoDB ${ctx.mongoClient ? '✅' : '❌'}
...
export async function handleMetricas(ctx) {
  ...
  try {
    ...
    console.error('[Metricas] Failed to load snapshots:', err.message);
  ...
  const rows = ['Novos', 'Ativos', 'Retorn.', 'Msgs', 'Trans.', 'Dividas'];
...
export async function handleRetencao(ctx) {
  ...
  try {
    ...
    console.error('[Retencao] getRetentionData error:', err.message, err.stack);
...
export async function handleExportar(ctx) {
  ...
  if (isJson) {
    ...
    const safeJson = jsonStr.length > MAX_WHATSAPP_CHARS
      ? jsonStr.substring(0, MAX_WHATSAPP_CHARS - 20) + '\n...(truncado)'
      : jsonStr;
    await ctx.replyWithRetry(safeJson);
    return;
  }
