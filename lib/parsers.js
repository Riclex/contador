import { MAX_AMOUNT } from './security.js';
...
function parseTransactionRegex(text) {
  ...
  if (!amount || isNaN(amount) || amount <= 0 || amount > MAX_AMOUNT) {
...
function parseDebtRegex(text) {
  ...
  const isValidDebtAmount = (amt) => Number.isFinite(amt) && amt > 0 && amt <= MAX_AMOUNT;
  ...
  if (match1) {
    ...
    if (!isValidDebtAmount(amount)) return { error: 'ambiguous' };
    return {
      type: "recebido",
      creditor: "user",
      debtor: match1[1],
  ...
  if (match2) {
    const amount = parseAmount(match2[1]);
    if (!isValidDebtAmount(amount)) return { error: 'ambiguous' };
    return {
      type: "recebido",
      creditor: "user",
  ...
  if (match4) {
    const amount = parseAmount(match4[1]);
    if (!isValidDebtAmount(amount)) return { error: 'ambiguous' };
    return {
      type: "devido",
      creditor: match4[3],
  ...
  if (match5) {
    const amount = parseAmount(match5[1]);
    if (!isValidDebtAmount(amount)) return { error: 'ambiguous' };
    return {
      type: "recebido",
      creditor: "user",
