...
export const TIPS = [
  ...
  { id: 'negocio-1', category: 'negocio', text: 'Saber quanto vendes não é o mesmo que saber quanto ganhas. Retira as despesas e o que reinvestes para ver o lucro real, e é ele que diz se vale a pena continuar.' },
];
...
// One-line tutorial-video link shown on the first-run welcome and /ajuda.
// Operator sets TUTORIAL_VIDEO_URL after uploading the video; until then this
// returns '' so no dead link is shipped to users.
export function tutorialVideoLine() {
  return process.env.TUTORIAL_VIDEO_URL
    ? `\u{1F39C} Como funciona (1 min): ${process.env.TUTORIAL_VIDEO_URL}`
    : '';
}