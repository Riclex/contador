...
export async function handleAnunciar(ctx) {
  ...
  // Get all consented users from onboarding collection
  const consentedUsers = await ctx.db.collection('onboarding').find({ state: 'completed' }, { projection: { phone: 1 } }).toArray();
  if (consentedUsers.length === 0) {
    await ctx.reply("Nenhum utilizador consentido encontrado.");
    return;
  }

  // Store announcement for async delivery — the webhook handler will send to each user
  // For MVP, we send directly via Twilio using the broadcast function from ctx
  const message = `\u{1F4E2} Anúncio do Contador\n\n${announcement}`;
  if (ctx.broadcast) {
    const result = await ctx.broadcast(consentedUsers, message);
    await ctx.reply(`Anúncio enviado: ${result.sent} utilizadores${result.failed > 0 ? ` (${result.failed} falharam)` : ''}`);
  } else {
    // Fallback: just confirm the announcement was logged
    await ctx.reply(`Anúncio registado para ${consentedUsers.length} utilizadores. Entrega em breve.`);
    await ctx.logEvent('announcement_queued', { user_count: consentedUsers.length, text_preview: announcement.substring(0, 100) });
  }
}
