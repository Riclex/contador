...
export function createWebhookHandler(deps) {
  return async (req, res) => {
    ...
    const {
      ...
      commandHandlers, stateHandlers, parseHandlers,
      ...
      twilioClient, twilio,
      hashPhone, isValidWhatsAppPhone, sanitizeInput, normalize,
      isAffirmative, isConfirmationWord, SessionState, OnboardingState,
      logEventFailures,
    } = deps;

    // Nested handler destructuring
    const { handleHoje, handleQuemedeve, handleQuemdevo, handleKilapi, handlePago, handleStats, handleRetencao, handleAnunciar, handleAjuda, handlePrivacidade, handleTermos, handleMeusdados, handleApagar, handleDesfazer, handleResumo, handleMes, handleFeedback, handleExportar, handleMetricas } = commandHandlers;
