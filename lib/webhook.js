...
export function createWebhookHandler(deps) {
  return async (req, res) => {
    ...
    const { handleAwaitingConfirmation, handleAwaitingDebtConfirmation, handleAwaitingPagoConfirm, handleAwaitingDebtorName, handleAwaitingApagarConfirm, handleAwaitingDesfazerConfirm, handleAwaitingReferralName, handleAwaitingReferralPhone } = stateHandlers;
    ...
    // Input sanitization. Keep the sanitized-but-not-lowercased text as `rawText` so
    // handlers that need original casing (e.g. referral names) can read it; `text` is
    // the normalized (lowercased) form used for command/state dispatch.
    const sanitizedText = sanitizeInput(rawText);
    const text = normalize(sanitizedText);
    ...
    try {
    // Retry protection — dedup BEFORE rate limiting so Twilio retries don't consume the
    // user's daily message budget, and don't re-log message_sent below.
    if (!messageSid) {
    ...
    if (onboardingState === OnboardingState.AWAITING_CONSENT) {
      if (isAffirmative(text)) {
        ...
        // If this person was referred, mark the referral activated (fire-and-forget).
        try {
          await activateReferral(deps.referrals, userHash);
        } catch (err) {
          logger.error(err, '[WEBHOOK] activateReferral error');
        }
        await replyWithRetry(from, `Perfeito! Estás pronto. ✅

  Manda AGORA esta mensagem para veres como funciona:
  👉 "vendi 500 de pão"

  Depois manda "hoje" para ver o teu saldo. 📊`);
        return res.sendStatus(204);
      } else {
    ...
    // Log message_sent event (after consent check — only for consenting users; dedup
    // already ran above, so Twilio retries never reach this line).
    ...
    async function saveSessionIfDirty() {
      if (sessionDirty) {
        try {
          await setSession(db, deps.mongoConnected, from, sessions.get(sessionKey));
          sessionDirty = false;
        } catch (err) {
    ...
    const ctx = {
      ...
      referrals,
      mongoClient: mongo,
      mongoConnected: deps.mongoConnected,
    ...
    case SessionState.AWAITING_REFERRAL_NAME:
      await handleAwaitingReferralName(ctx);
      return res.sendStatus(204);

    case SessionState.AWAITING_REFERRAL_PHONE:
